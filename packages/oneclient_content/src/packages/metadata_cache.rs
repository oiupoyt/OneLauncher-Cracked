...
#[tracing::instrument(level = "debug", skip(ctx, project_ids))]
pub async fn read_cached_package_meta(
    ...
    let rows = meta_dao::get_package_metadata_batch(&ctx.db, provider as i64, project_ids)
        .await
        .unwrap_or_default();
...
pub async fn cached_project_detail(
    ...
    let cached =
        read_cached_package_meta(ctx, provider, std::slice::from_ref(&project_id.to_string()))
            .await;
...
pub async fn get_version_cached(
    ...
    if let Some(cached) = cached_version_detail(ctx, provider, project_id, version_id).await {
        return Ok(cached);
    }
...
async fn cached_version_detail(
    ...
    let release =
        artifact_dao::get_provider_release(&ctx.db, provider as i64, project_id, version_id)
            .await
            .ok()
            .flatten()?;
    let artifact = artifact_dao::get_artifact_by_hash(&ctx.db, &release.hash)
        .await
        .ok()
        .flatten()?;
...
pub async fn fetch_package_meta(
    ...
    provider: ProviderId,
    project_ids: &[String],
) -> ContentResult<HashMap<String, CachedPackageMeta>> {
    if project_ids.is_empty() {
    ...
    for row in meta_dao::get_package_metadata_batch(&ctx.db, provider_repr, project_ids).await? {
        out.insert(
            row.project_id.clone(),
            CachedPackageMeta {
                provider,
    ...
    tracing::debug!(
        count = missing.len(),
        "fetching missing package metadata from provider"
    );

    let details = ctx
        .providers
        .get(provider)?
        .get_projects(&missing, ctx)
        .await?;
