...
pub async fn recover_sessions(state: &Arc<LauncherState>) {
    ...
    tracing::info!(
        count = sessions.len(),
        "recovering unfinished game sessions"
    );
...
async fn classify(
    ...
    let Ok(cluster) = state.clusters.get(cluster_id).await else {
        tracing::warn!(
            cluster_id,
            "cluster missing for unfinished session; closing it"
        );
        close_untraceable(state, &session).await;
        return None;
    };
...
async fn readopt(
    ...
    state
        .services
        .events
        .game_stage(cluster_id, LaunchStage::Running);
...
async fn reconcile(
    ...
    if shared_dir_busy {
        tracing::debug!(
            cluster_id,
            "shared game dir still in use; skipping exit cleanup"
        );
    }
